...
> 以下为**目标接口设计**。当前代码已落地 `FixedPool` 与 `HashKVStore` 的具体类型（见 §4），但尚未抽出 `Allocator` / `KVStore` 抽象基类——两者各自仍是唯一实现。按渐进式策略，抽象基类留到引入**第二个实现**、确实需要运行时替换时再加入；在此之前保持具体类型，避免不必要的虚函数开销与抽象。
...
| 1a | `FixedPool` 实现 | 构造 / allocate / deallocate / 析构 | ✅ 完成 |
| 1b | `HashKVStore` | 哈希表（separate chaining + FNV-1a），用 FixedPool 管 entry 节点 | ✅ 完成 |
...
├── CLAUDE.md                   # 给 Claude Code 的项目指引（本地文件，不入 git）
├── include/mempool/            # 公开头文件
│   ├── fixed_pool.h            # FixedPool 接口
│   └── hash_kv_store.h         # HashKVStore 接口 + Entry 定义
├── src/
│   ├── CMakeLists.txt          # 库 / 可执行目标
│   ├── main.cpp                # Demo 入口
│   └── mempool/
│       ├── fixed_pool.cpp      # FixedPool 实现
│       └── hash_kv_store.cpp   # HashKVStore 实现
├── tests/
│   ├── CMakeLists.txt
│   ├── test_framework.h        # 零依赖断言宏
│   ├── test_smoke.cpp          # FixedPool 冒烟测试
│   └── test_hash_kv_store.cpp  # HashKVStore 测试（基本/冲突/OOM/计数）
└── docs/
    ├── README.md
    └── architecture.md         # 本文件
```
...
阶段 0 / 1a / 1b 完成：`FixedPool` 与 `HashKVStore` 均已实现并有测试（冒烟、哈希冲突、OOM、长度守卫、计数）。下一步：阶段 2，`SlabAllocator`（多档固定大小池）。详见 §5。
